...
def get_sensorId_dict():
	return name_to_sensorId_dict#


def get_name_dict():
	return sensorId_to_name_dict


def get_sorted_list():
	sensorId_key = []
	for item in name_to_sensorId_dict:
		sensorId_key.append(item)
		sorted(sensorId_key)
...
def get_sensor_area(sensorId):
	row = get_sensor_row(sensorId)
	if row <= 14:
		return 'top'
	elif row <= 28:
		return 'mid'
